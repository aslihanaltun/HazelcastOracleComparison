...
public class App 
{
    ...
    public static IList<Integer> hzStock = hazelcast.getList("randomNumbers");
//connection yap
    ...
    public static long orcPut(int n){
        ...
        String orcUser= "root";
        String orcPassword ="123";

        try {
            Connection con = DriverManager.getConnection(
                    orcURL, orcUser, orcPassword);
            PreparedStatement pstmt = con.prepareStatement("INSERT INTO RandomNumbers VALUES(?,?)");
    ...
    public static long orcGet(int n){
        ...
        try {
            ...
            stopWatch.start();
            for(int i = 0; i<n; i++) {
                PreparedStatement pstmt = con.prepareStatement("SELECT * FROM RandomNumbers WHERE RandomNumbers[0]= "+i);
                int random = rand.nextInt();
                pstmt.setInt(i, random);
                pstmt.execute();
            }
